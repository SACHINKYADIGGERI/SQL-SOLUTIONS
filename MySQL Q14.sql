...
--SOLUTION 1 WITH (INBULIT FUNCTION)

select
      generate_series(min(serial_no), max(serial_no)) as missing_data
from 
      invoice
except
select
      serial_no from invoice
order by missing_data ASC


--SOLUTION 2 WITH (RECRESSION FUNCTION)
